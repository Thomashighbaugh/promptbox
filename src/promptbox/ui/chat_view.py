...
def render_chat_ui(llm_service: LLMService, chat_service: ChatService):
    ...
    if not item_name and not st.session_state.get("loading_session_flag", False):
        ...
        if st.button("Go Back", key="button"):
    ...
    if "current_messages_data" not in st.session_state:
...
def render_chat_setup_stage(llm_service: LLMService, active_prompt: Optional[PromptData], active_card: Optional[CharacterCardData], item_name: str):
    ...
    if not available_models:
        st.error("No LLM providers are configured. Please check your API key setup.")
        return
    ...
    if current_provider not in available_models:
    ...
    if active_prompt:
        ...
        if active_prompt.system_instruction:
            initial_text_parts.append(active_prompt.system_instruction)
        if active_prompt.user_instruction:
            initial_text_parts.append(active_prompt.user_instruction)
        if active_prompt.assistant_instruction:
    ...
    st.markdown("---")
    if st.button("🚀 Start Chat", type="primary", use_container_width=True, disabled=not current_model, key="button1"):
...
def render_chatting_stage(llm_service: LLMService, chat_service: ChatService, item_name: str, active_prompt: Optional[PromptData], active_card: Optional[CharacterCardData]):
    ...
    with btn_cols[0]:
        if st.button("💾 Save Session", use_container_width=True, help="Save/Update the current chat.", key="button2"):
            _save_current_chat_session(chat_service, item_name, active_prompt, active_card)
            st.toast("Chat session saved/updated!", icon="✅")
            st.rerun()

    with btn_cols[1]:
        if st.button("📝 Export MD", use_container_width=True, help="Export current chat to Markdown.", key="button3"):
            if st.session_state.current_chat_session_id:
    ...
    with btn_cols[2]:
        if st.button("⚙️ Change Model", use_container_width=True, help="Change LLM for this chat.", key="button4"):
            st.session_state.next_chat_stage = "setup"
            st.session_state.chat_stage = "ask_save_dialog"
            st.rerun()

    with btn_cols[3]:
        if st.button("🛑 Exit Chat", type="secondary", use_container_width=True, help="Exit chat. You'll be asked to save.", key="button5"):
    ...
    for i, msg_data in enumerate(st.session_state.current_messages_data):
        ...
        with st.chat_message(role_for_display, key=f"chat_message_{i}"):
    ...
    if not llm:
        st.error(f"Failed to init model '{st.session_state.chat_model}'. Try changing model or check logs.")
        return

    if user_input := st.chat_input("Your message...", key="chat_input"):
    ...
    if st.session_state.current_messages_data and st.session_state.current_messages_data[-1].role == "user":
        ...
        with st.chat_message("assistant", key="chat_message"):
...
def _save_current_chat_session(chat_service: ChatService, item_name: str, active_prompt: Optional[PromptData], active_card: Optional[CharacterCardData]):
    ...
    if session_id:
        ...
        if not origin_prompt_id and not origin_card_id and st.session_state.get("chat_origin_item_id"):
            ...
            if origin_id_str.startswith("prompt_"):
                origin_prompt_id = int(origin_id_str.split("_")[1])
            elif origin_id_str.startswith("card_"):
                origin_card_id = int(origin_id_str.split("_")[1])
...
def render_ask_save_dialog(chat_service: ChatService, item_name:str, active_prompt: Optional[PromptData], active_card: Optional[CharacterCardData]):
    ...
    with col1:
        if st.button("✅ Yes, Save", use_container_width=True, type="primary", key="button6"):
            _save_current_chat_session(chat_service, item_name, active_prompt, active_card)
            ...
            st.rerun()
    with col2:
        if st.button("❌ No, Discard", use_container_width=True, key="button7"):
            next_stage_or_view = st.session_state.get("next_chat_stage", "home")
            if next_stage_or_view in ["setup", "chatting"]:
                st.session_state.chat_stage = next_stage_or_view
            else:
            ...
            st.rerun()
    with col3:
        if st.button("↩️ Cancel", use_container_width=True, key="button8"):
            st.session_state.chat_stage = "chatting"
...
def _export_unsaved_chat_to_markdown(item_name: str):
    ...
    if not messages_data:
        st.warning("Nothing to export.", key="warning1")
        return
    ...
    try:
        ...
        st.success(f"Unsaved chat exported successfully to: `{save_path}`", key="success2")
    except Exception as e:
